...
#include "fstPedNoise.h"
  ...
  while (!in2.eof()){
    ...
    if(timebin == usedTB) {
      ...
      cout<<"         row.cmNoise["<<chipId<<"]="<<100*(int)cmNoiseT<<";"<<endl;
    }
  }
  in2.close();
  ...
  while (!in.eof()){
    ...
    if(timebin == usedTB) {
      cout<<"	row.pedestal["<<channelId<<"]="<<pedestalT<<";"<<endl;
      cout<<"	row.rmsNoise["<<channelId<<"]="<<100*(int)rmsT<<";"<<endl;
    }
  }
  in.close();
