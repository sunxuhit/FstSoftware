void write_hssOnTpc()
{
  ...
  for (Int_t i = 0; i < MAX_DB_INDEX; i++) {
    ...
    if(i == 0) strcpy(table[i].comment,"initial commit: FST Left Half");
    if(i == 1) strcpy(table[i].comment,"initial commit: FST Right Half");
  }
