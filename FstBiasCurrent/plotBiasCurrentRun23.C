...
void plotBiasCurrentRun23(int mpodId = 2)
{
  // read in inner sensor of Disk2 module 4: isegId = 6
  const int iIsegId = 6;
  string inputfileIS = Form("./data/fstmpod0%d_u%d_run23.txt", mpodId, iIsegId);
  ...
  const int oIsegId = 7;
  string inputfileOS = Form("./data/fstmpod0%d_u%d_run23.txt", mpodId, oIsegId);
  ...
  h_play->SetTitle("Leak Current of FST Disk2 Module4 in Run23");
  h_play->SetStats(0);
  ...
  h_play->GetXaxis()->SetLimits(TDatime(2023, 4, 01, 0, 0, 0).Convert(), TDatime(2023, 8, 31, 0, 0, 0).Convert());
  ...
  long tsShift  = -60*60*24*10; // shift 2 days
  const long tsCosmic = TDatime(2023, 4, 27, 0, 0, 0).Convert();
  TLatex *lCosmic;
  ...
  tsShift  = -60*60*24*8; // shift 2 days
  const long tsBeam = TDatime(2023, 5, 18, 0, 0, 0).Convert();
  TLatex *lBeam;
  lBeam = new TLatex(tsBeam+tsShift, 15.7,"Beam Start");
  lBeam->SetTextFont(42);
  lBeam->SetTextSize(0.04);
  lBeam->SetTextColor(kRed-7);
  // lBeam->SetLineWidth(2);
  lBeam->Draw();
  TArrow *aBeam = new TArrow(tsBeam, 12.5, tsBeam, 15.5, 0.010,"<|");
  ...
  const long tsRunStop = TDatime(2023, 8, 4, 12, 0, 0).Convert();
  TLatex *lRunStop;
  lRunStop = new TLatex(tsRunStop-tsShift, 20.7,"Run23 Stop");
  ...
  c_bCurrent->SaveAs("./figures/Run23LeakCurrent.eps");
  c_bCurrent->SaveAs("./figures/Run23LeakCurrent.png");
}
