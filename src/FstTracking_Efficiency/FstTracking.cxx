...
void FstTracking::initEfficiency_Hits()
{
  ...
  v_mMatchedHitEfficiency = new TVectorT<double>(8);
}
...
void FstTracking::calEfficiency_Hits(FstEvent *fstEvent)
{
  ...
  for(int i_track = 0; i_track < trackHitVec.size(); ++i_track)
  {
    ...
    //Projected counts and actual counts for time sequence study
    if(r_proj <= rMaxFst && r_proj >= rMinFst && phi_proj <= phiMaxFst && phi_proj >= phiMinFst)
    { // used for efficiency only if the projected position is within FST detector area
      for(int i_match = 0; i_match < 8; ++i_match)
      {
	nProjected[0][i_match]++;
	int nMatchedTrack = 0;
	if(numOfFstHits > 0)
	{
	  for(int i_hit = 0; i_hit < numOfFstHits; ++i_hit)
	  { // loop over all possible hits
	    ...
	    if(i_match == 0 && r_fst <= rMaxFst && r_fst >= rMinFst && phi_fst <= phiMaxFst && phi_fst >= phiMinFst)
	    {
	      nMatchedTrack++;
	    }
	...
	if(nMatchedTrack > 0) nMatched[0][i_match]++;
      }
    }
...
void FstTracking::writeEfficiency_Hits()
{
  ...
  v_mMatchedHitEfficiency->Write("v_mMatchedHitEfficiency");
}
//--------------Efficiency with Hits---------------------
...
void FstTracking::initEfficiency_Clusters()
{
  ...
  v_mMatchedSimpleEfficiency_2Layer = new TVectorT<double>(8);
  v_mMatchedSimpleEfficiency_3Layer = new TVectorT<double>(8);
  v_mMatchedScanEfficiency_2Layer = new TVectorT<double>(8);
  v_mMatchedScanEfficiency_3Layer = new TVectorT<double>(8);
}
  ...
  { // only event with 1 track is used
	...
	{ // used for efficiency only if the projected position is within FST acceptance
	  for(int i_match = 0; i_match < 8; ++i_match)
	  {
	    ...
	    if(nMatchedTrack > 0) nMatched[1][i_match]++;
	  }
	}
    ...
    for(int i_track = 0; i_track < trackClusterVec.size(); ++i_track)
    {
      ...
      {
        if(r_proj <= rMaxFst && r_proj >= rMinFst && phi_proj <= phiMaxFst && phi_proj >= phiMinFst)
	{ // used for efficiency only if the projected position is within FST detector
	  if(clusterVec_ist2.size() > 0)
	  {
	    ...
	    for(int i_match = 0; i_match < 8; ++i_match)
	    {
	      if(nMatchedTrack_IST2 > 0) nProjected[2][i_match]++;
	      int nMatchedTrack = 0;
	      if(clusterVec_fst.size() > 0)
	      {
		for(int i_cluster = 0; i_cluster < clusterVec_fst.size(); ++i_cluster)
		{ // loop over all possible clusters
	      ...
	      if(nMatchedTrack_IST2 > 0 && nMatchedTrack > 0) nMatched[2][i_match]++;
	    }
	  }
	}
  ...
  { // only event with 1 track is used
	...
	{ // used for efficiency only if the projected position is within FST detector
	  for(int i_match = 0; i_match < 8; ++i_match)
	  {
	    ...
	    if(nMatchedTrack > 0) nMatched[3][i_match]++;
	  }
	}
	...
	{ // used for efficiency only if the projected position is within FST acceptance
	  if(clusterVec_ist2.size() > 0)
	  {
	    ...
	    for(int i_match = 0; i_match < 8; ++i_match)
	    {
	      if(nMatchedTrack_IST2 > 0) nProjected[4][i_match]++;
	      int nMatchedTrack = 0;
	      if(clusterVec_fst.size() > 0)
	      {
		for(int i_cluster = 0; i_cluster < clusterVec_fst.size(); ++i_cluster)
		{ // loop over all possible clusters
	      ...
	      if(nMatchedTrack_IST2 > 0 && nMatchedTrack > 0) nMatched[4][i_match]++;
	    }
	  }
	}
...
void FstTracking::writeEfficiency_Clusters()
{
  ...
  v_mMatchedSimpleEfficiency_2Layer->Write("v_mMatchedSimpleEfficiency_2Layer");
  v_mMatchedSimpleEfficiency_3Layer->Write("v_mMatchedSimpleEfficiency_3Layer");
  v_mMatchedScanEfficiency_2Layer->Write("v_mMatchedScanEfficiency_2Layer");
  v_mMatchedScanEfficiency_3Layer->Write("v_mMatchedScanEfficiency_3Layer");
}

//--------------Fill Efficiency Containers---------------
void FstTracking::fillContainers()
{ 
  // type 0 = hits | 1 = simple clusters (2layer) | 2 = simple clusters (3layer) | 
  // type 3 = scan clusters (2layer) | 4 = scan clusters (3layer)
  for(int i_type = 0; i_type < 5; ++i_type)
  {
    double effAry[8] = {0.0};
    for(int i_match = 0; i_match < 8; ++i_match)
    {
      effAry[i_match] = (double)nMatched[i_type][i_match]/(double)nProjected[i_type][i_match];
    }
    if(i_type == 0)
    {  
      v_mMatchedHitEfficiency->SetElements(effAry); 
    }
    else if(i_type == 1)
    {
      v_mMatchedSimpleEfficiency_2Layer->SetElements(effAry);
    }
    else if(i_type == 2)
    {
      v_mMatchedSimpleEfficiency_3Layer->SetElements(effAry);
    }
    else if(i_type == 3)
    {
      v_mMatchedScanEfficiency_2Layer->SetElements(effAry);
    }
    else if(i_type == 4)
    {
      v_mMatchedScanEfficiency_3Layer->SetElements(effAry);
    }
  }
}
//--------------Fill Efficiency Containers---------------
