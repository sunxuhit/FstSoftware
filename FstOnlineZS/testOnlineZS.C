  ...
  for(int i_rdo = 0; i_rdo < 6; ++i_rdo)
  {
	...
	for(int i_ch = 0; i_ch < 128; ++i_ch)
	{
	  for(int i_tb = 0; i_tb < 9; ++i_tb)
	  {
	    ...
	    float totStdDev[6][3][24][128][9];
	    float ranStdDev[6][3][24][128][9];
	    file_pedestal << i_rdo << "    " << i_arm << "    "  << i_apv << "    "  << i_ch << "    "  << i_tb << "    "  << pedestal[i_rdo][i_arm][i_apv][i_ch][i_tb] << "    "  << totStdDev[i_rdo][i_arm][i_apv][i_ch][i_tb] << "    "  << ranStdDev[i_rdo][i_arm][i_apv][i_ch][i_tb] << endl;
	  }
	}
      }
    }
