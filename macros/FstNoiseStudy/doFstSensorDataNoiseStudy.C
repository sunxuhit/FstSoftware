...
int doFstSensorDataNoiseStudy(string module = "Mod03", string hv = "HV70V", string date = "07122020")
{
  ...
  std::string inputlist = Form("../../list/FST/timesequence/FstData_%s_%s_%s.list",module.c_str(),hv.c_str(),date.c_str());
  cout << "input list set to: " << inputlist.c_str() << endl;

  std::string outputfile = Form("../../output/noise/FstDataNoise_%s_%s_%s.root",module.c_str(),hv.c_str(),date.c_str());
