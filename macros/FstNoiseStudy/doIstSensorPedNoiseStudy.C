...
int doIstSensorPedNoiseStudy(string module = "Mod36", string hv = "HV60V")
{
  std::cout << "gSystem::Load <- libFstNoiseStudy.dylib" << endl;

  FstNoiseStudy *fst = new FstNoiseStudy();

  std::string inputlist = Form("../../list//IST/IstPed_%s_%s.list",module.c_str(),hv.c_str());
  cout << "input list set to: " << inputlist.c_str() << endl;

  std::string outputfile = Form("../../output/noise/Ist%s/IstPedNoise_%s_%s.root",module.c_str(),module.c_str(),hv.c_str());
