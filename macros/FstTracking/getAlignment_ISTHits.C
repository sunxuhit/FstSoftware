...
int getAlignment_ISTHits()
{
  ...
  for(int i_event = 0; i_event < NumOfEvents; ++i_event)
  {
    ...
    if(hitVec_ist2.size() > 0) // IST2 has at least one hit
    {
      ...
      {
	double x1 = hitVec_ist1[i_ist1]->getPosX();
	double y1 = hitVec_ist1[i_ist1]->getPosY();
	...
	{
	  double x3 = hitVec_ist3[i_ist3]->getPosX();
	  double y3 = hitVec_ist3[i_ist3]->getPosY();
	  ...
	  {
	    double x2 = hitVec_ist2[i_ist2]->getPosX();
	    double y2 = hitVec_ist2[i_ist2]->getPosY();
...
tPars minuitAlignment(dVec x2_orig, dVec y2_orig, dVec x1_orig, dVec y1_orig, dVec x3_orig, dVec y3_orig, tPars cParameters)
{
  ...
  for(int i_hit = 0; i_hit < x1_orig.size(); ++i_hit)
  { // set hit containers
    x2_temp.push_back(x2_orig[i_hit]);
    y2_temp.push_back(y2_orig[i_hit]);
  ...
  int numOfUsedHits = 0;
  for(int i_hit = 0; i_hit < x1_temp.size(); i_hit++)
  {
    double x1_corr_temp = x1_temp[i_hit]*TMath::Cos(phi_ist1_temp) + y1_temp[i_hit]*TMath::Sin(phi_ist1_temp) + x1_shift_temp;
    double y1_corr_temp = y1_temp[i_hit]*TMath::Cos(phi_ist1_temp) - x1_temp[i_hit]*TMath::Sin(phi_ist1_temp) + y1_shift_temp;

    double x3_corr_temp = x3_temp[i_hit]*TMath::Cos(phi_ist3_temp) + y3_temp[i_hit]*TMath::Sin(phi_ist3_temp) + x3_shift_temp;
    double y3_corr_temp = y3_temp[i_hit]*TMath::Cos(phi_ist3_temp) - x3_temp[i_hit]*TMath::Sin(phi_ist3_temp) + y3_shift_temp;
    ...
    double dx = x2_temp[i_hit] - x2_proj_temp;
    double dy = y2_temp[i_hit] - y2_proj_temp;
    // cout << "dx = " << dx << ", dy = " << dy << endl;

    if(abs(dx) < xCut_temp && abs(dy) < yCut_temp)
    {
      x2_fit.push_back(x2_temp[i_hit]);
      y2_fit.push_back(y2_temp[i_hit]);
  ...
  { //minimisation function computing the sum of squares of residuals
    ...
    {
      double x1_corr = x1_fit[i_hit]*TMath::Cos(phi_rot_ist1) + y1_fit[i_hit]*TMath::Sin(phi_rot_ist1) + x1_shift;
      double y1_corr = y1_fit[i_hit]*TMath::Cos(phi_rot_ist1) - x1_fit[i_hit]*TMath::Sin(phi_rot_ist1) + y1_shift;

      double x3_corr = x3_fit[i_hit]*TMath::Cos(phi_rot_ist3) + y3_fit[i_hit]*TMath::Sin(phi_rot_ist3) + x3_shift;
      double y3_corr = y3_fit[i_hit]*TMath::Cos(phi_rot_ist3) - x3_fit[i_hit]*TMath::Sin(phi_rot_ist3) + y3_shift;
      ...
      double dx = x2_fit[i_hit] - x2_proj;
      double dy = y2_fit[i_hit] - y2_proj;
  ...
  for (int i_hit = 0; i_hit < numOfUsedHits; i_hit++)
  {
    double x1_corr_MF = x1_fit[i_hit]*TMath::Cos(phi_rot_ist1_MF) + y1_fit[i_hit]*TMath::Sin(phi_rot_ist1_MF) + x1_shift_MF;
    double y1_corr_MF = y1_fit[i_hit]*TMath::Cos(phi_rot_ist1_MF) - x1_fit[i_hit]*TMath::Sin(phi_rot_ist1_MF) + y1_shift_MF;

    double x3_corr_MF = x3_fit[i_hit]*TMath::Cos(phi_rot_ist3_MF) + y3_fit[i_hit]*TMath::Sin(phi_rot_ist3_MF) + x3_shift_MF;
    double y3_corr_MF = y3_fit[i_hit]*TMath::Cos(phi_rot_ist3_MF) - x3_fit[i_hit]*TMath::Sin(phi_rot_ist3_MF) + y3_shift_MF;
    ...
    double dx = x2_fit[i_hit] - x2_proj_MF;
    double dy = y2_fit[i_hit] - y2_proj_MF;
