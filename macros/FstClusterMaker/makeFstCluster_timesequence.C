...
int makeFstCluster_timesequence(string hv = "HV200V", bool isSavePed = true, bool isApplyCMNCorr = false, float nFstHitsCut = 4.5, int numOfUsedTimeBins = 3, float nFstThresholdCut = 3.0, string date = "04022020")
{
  ...
  std::string outputfile = Form("../../output/clusters/FstClusters_%s_Th%1.1fTb%dPed%1.1f_%s_%s_%s.root",hv.c_str(),nFstHitsCut,numOfUsedTimeBins,nFstThresholdCut,pedMode.c_str(),cmnMode.c_str(),date.c_str());
