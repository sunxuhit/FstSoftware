...
int makeFstCluster(string hv = "HV140V", string config = "Th4o5Tb3", bool isSavePed = true, bool isApplyCMNCorr = true, float nFstHitsCut = 4.5, int numOfUsedTimeBins = 3, float nFstThresholdCut = 2.0)
{
  ...
  if(isSavePed) outputfile = Form("../../output/configuration/FstClusters_%s_withPed_%s.root",hv.c_str(),config.c_str());
  if(!isSavePed) outputfile = Form("../../output/configuration/FstClusters_%s_woPed_%s.root",hv.c_str(),config.c_str());
