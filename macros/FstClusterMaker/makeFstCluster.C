...
int makeFstCluster(string mod = "Mod03", string hv = "HV70V", bool isSavePed = true, bool isApplyCMNCorr = true, float nFstHitsCut = 4.0, int numOfUsedTimeBins = 2, float nFstThresholdCut2 = 2.5, float nFstThresholdCut1 = 3.5, string date = "07122020")
{
  ...
  std::string inputlist = Form("../../list/FST/timesequence/FstData_%s_%s_%s.list",mod.c_str(),hv.c_str(),date.c_str());
  cout << "input list set to: " << inputlist.c_str() << endl;

  std::string outputfile = Form("../../output/clusters/FstClusters_%s_%s_Th%1.1fTb%dPed%1.1fPed%1.1f_%s_%s_%s.root",mod.c_str(),hv.c_str(),nFstHitsCut,numOfUsedTimeBins,nFstThresholdCut2,nFstThresholdCut1,pedMode.c_str(),cmnMode.c_str(),date.c_str());
