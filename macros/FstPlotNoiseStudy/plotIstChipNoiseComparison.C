...
void plotIstChipNoiseComparison(string module = "Mod03", string istMod = "Mod36")
{
  ...
  for(int i_rstrip = 0; i_rstrip < 4; ++i_rstrip)
  {
    ...
    h_mMeanPedSigma_RStrip[i_rstrip] = (TH1F*)File_InPutInnerFst->Get(HistName.c_str());

    HistName = Form("h_mMeanCMNSigma_RStrip%d",i_rstrip);
    h_mMeanCMNSigma_RStrip[i_rstrip] = (TH1F*)File_InPutInnerFst->Get(HistName.c_str());

    HistName = Form("h_mMeanRanSigma_RStrip%d",i_rstrip);
    h_mMeanRanSigma_RStrip[i_rstrip] = (TH1F*)File_InPutInnerFst->Get(HistName.c_str());
  }
  ...
  for(int i_rstrip = 4; i_rstrip < 8; ++i_rstrip)
  {
    string HistName; 
    HistName = Form("h_mMeanPedSigma_RStrip%d",i_rstrip);
    h_mMeanPedSigma_RStrip[i_rstrip] = (TH1F*)File_InPutOuterFst->Get(HistName.c_str());

    HistName = Form("h_mMeanCMNSigma_RStrip%d",i_rstrip);
    h_mMeanCMNSigma_RStrip[i_rstrip] = (TH1F*)File_InPutOuterFst->Get(HistName.c_str());

    HistName = Form("h_mMeanRanSigma_RStrip%d",i_rstrip);
    h_mMeanRanSigma_RStrip[i_rstrip] = (TH1F*)File_InPutOuterFst->Get(HistName.c_str());
  }
