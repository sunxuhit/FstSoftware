...
{
  const int numOfTS = 38;
  string date[numOfTS] = {"0329","0330","0331","0402","0403","0404","0406","0407","0408","0413","0417","0423","0427","0429","0502","0504","0507","0509","0511","0512","0514","0516","0518","0519","0520","0521","0525","0526","0527","0528","0529","0531","0601","0602_0","0602_1","0603","0604_0","0604_1"};

  TH1F *h_mMeanSignalHits_Rstrip[4];
  TH1F *h_mMeanNoiseHits_Rstrip[4];
  TH1F *h_mMeanSNRatioHits_Rstrip[4];
  TH1F *h_mMeanMaxTbHits_Rstrip[4];
  ...
  for(int i_rstrip = 0; i_rstrip < 4; ++i_rstrip)
  {
    ...
    h_mMeanSignalHits_Rstrip[i_rstrip] = new TH1F(HistName.c_str(),HistName.c_str(),numOfTS,-0.5,numOfTS-0.5);
    HistName = Form("h_mMeanNoiseHits_Rstrip%d",i_rstrip);
    h_mMeanNoiseHits_Rstrip[i_rstrip] = new TH1F(HistName.c_str(),HistName.c_str(),numOfTS,-0.5,numOfTS-0.5);
    HistName = Form("h_mMeanSNRatioHits_Rstrip%d",i_rstrip);
    h_mMeanSNRatioHits_Rstrip[i_rstrip] = new TH1F(HistName.c_str(),HistName.c_str(),numOfTS,-0.5,numOfTS-0.5);
    HistName = Form("h_mMeanMaxTbHits_Rstrip%d",i_rstrip);
    h_mMeanMaxTbHits_Rstrip[i_rstrip] = new TH1F(HistName.c_str(),HistName.c_str(),numOfTS,-0.5,numOfTS-0.5);
    HistName = Form("h_mMeanSignalClusters_Rstrip%d",i_rstrip);
    h_mMeanSignalClusters_Rstrip[i_rstrip] = new TH1F(HistName.c_str(),HistName.c_str(),numOfTS,-0.5,numOfTS-0.5);
  }

  TFile *File_InPut[numOfTS];
  TH1F *h_mSignalHits_Rstrip[numOfTS][4];
  TH1F *h_mNoiseHits_Rstrip[numOfTS][4];
  TH1F *h_mSNRatioHits_Rstrip[numOfTS][4];
  TH1F *h_mMaxTbHits_Rstrip[numOfTS][4];
  ...
  for(int i_ts = 0; i_ts < numOfTS; ++i_ts)
  {
    ...
    for(int i_rstrip = 0; i_rstrip < 4; ++i_rstrip)
    {
      ...
      h_mSignalHits_Rstrip[i_ts][i_rstrip] = (TH1F*)File_InPut[i_ts]->Get(HistName.c_str());
      HistName = Form("h_mNoiseHits_Rstrip%d",i_rstrip);
      h_mNoiseHits_Rstrip[i_ts][i_rstrip] = (TH1F*)File_InPut[i_ts]->Get(HistName.c_str());
      HistName = Form("h_mSNRatioHits_Rstrip%d",i_rstrip);
      h_mSNRatioHits_Rstrip[i_ts][i_rstrip] = (TH1F*)File_InPut[i_ts]->Get(HistName.c_str());
      HistName = Form("h_mMaxTbHits_Rstrip%d",i_rstrip);
      h_mMaxTbHits_Rstrip[i_ts][i_rstrip] = (TH1F*)File_InPut[i_ts]->Get(HistName.c_str());
      HistName = Form("h_mFstSimpleClustersSignal_Rstrip%d",i_rstrip);
      h_mFstSimpleClustersSignal_Rstrip[i_ts][i_rstrip] = (TH1F*)File_InPut[i_ts]->Get(HistName.c_str());
    }
  }

  string outputname = "./figures/SignalTS_140V.pdf";
  ...
  string output_start = "./figures/SignalTS_140V.pdf[";
  ...
  while(numOfUsedTS < numOfTS)
  {
	...
	for(int i_rstrip = 0; i_rstrip < 4; ++i_rstrip)
	{
	  ...
	  h_mFstSimpleClustersSignal_Rstrip[numOfUsedTS][i_rstrip]->SetStats(0);
	  h_mFstSimpleClustersSignal_Rstrip[numOfUsedTS][i_rstrip]->SetTitle(title.c_str());
	  h_mFstSimpleClustersSignal_Rstrip[numOfUsedTS][i_rstrip]->GetXaxis()->SetTitle("ADC");
	  h_mFstSimpleClustersSignal_Rstrip[numOfUsedTS][i_rstrip]->GetXaxis()->SetTitleSize(0.06);
	  h_mFstSimpleClustersSignal_Rstrip[numOfUsedTS][i_rstrip]->SetLineColor(i_rstrip+1);

	  if(i_rstrip == 0) h_mFstSimpleClustersSignal_Rstrip[numOfUsedTS][i_rstrip]->Draw();
	  else h_mFstSimpleClustersSignal_Rstrip[numOfUsedTS][i_rstrip]->Draw("same");

	  string LegName = Form("R_strip %d: Mean %1.2f",i_rstrip,h_mFstSimpleClustersSignal_Rstrip[numOfUsedTS][i_rstrip]->GetMean());
	  leg_FST->AddEntry(h_mFstSimpleClustersSignal_Rstrip[numOfUsedTS][i_rstrip],LegName.c_str(),"L");

	  double mean_orig    = h_mFstSimpleClustersSignal_Rstrip[0][i_rstrip]->GetMean();
	  // double err_orig     = h_mFstSimpleClustersSignal_Rstrip[0][i_rstrip]->GetMeanError();
	  double err_orig     = 0.0;
	  double mean_current = h_mFstSimpleClustersSignal_Rstrip[numOfUsedTS][i_rstrip]->GetMean();
	  double err_current  = h_mFstSimpleClustersSignal_Rstrip[numOfUsedTS][i_rstrip]->GetMeanError();
	  double ratio = mean_current/mean_orig;
	  double error = ErrDiv(mean_current, mean_orig, err_current, err_orig);
	  ...
	  // h_mMeanSignalClusters_Rstrip[i_rstrip]->SetBinContent(numOfUsedTS+1,h_mFstSimpleClustersSignal_Rstrip[numOfUsedTS][i_rstrip]->GetMean());
	}
	leg_FST->Draw("same");
      }
  ...
  for(int i_pad = 0;i_pad < 5; ++i_pad)
  {
    ...
    c_Signal_TS->cd(i_pad+1)->SetBottomMargin(0.20);
  ...
  c_Signal_TS->cd(3);
  {
	...
	h_mMeanNoiseHits_Rstrip[i_rstrip]->GetXaxis()->SetBinLabel(i_ts+1,date[i_ts].c_str());
      ...
      h_mMeanNoiseHits_Rstrip[i_rstrip]->GetYaxis()->SetRangeUser(0.5,1.5);
      h_mMeanNoiseHits_Rstrip[i_rstrip]->GetYaxis()->SetLabelSize(0.08);
      h_mMeanNoiseHits_Rstrip[i_rstrip]->GetYaxis()->SetNdivisions(505);
  ...
  c_Signal_TS->cd(4);
  {
	...
	h_mMeanSNRatioHits_Rstrip[i_rstrip]->GetXaxis()->SetBinLabel(i_ts+1,date[i_ts].c_str());
      ...
      h_mMeanSNRatioHits_Rstrip[i_rstrip]->GetYaxis()->SetRangeUser(0.5,1.5);
      h_mMeanSNRatioHits_Rstrip[i_rstrip]->GetYaxis()->SetLabelSize(0.08);
      h_mMeanSNRatioHits_Rstrip[i_rstrip]->GetYaxis()->SetNdivisions(505);
  ...
  c_Signal_TS->cd(5);
  {
    for(int i_rstrip = 0; i_rstrip < 4; ++i_rstrip)
    {
      h_mMeanMaxTbHits_Rstrip[i_rstrip]->SetTitle("");
      h_mMeanMaxTbHits_Rstrip[i_rstrip]->SetStats(0);
      for(int i_ts = 0; i_ts < numOfTS; ++i_ts)
      {
	h_mMeanMaxTbHits_Rstrip[i_rstrip]->GetXaxis()->SetBinLabel(i_ts+1,date[i_ts].c_str());
      ...
      h_mMeanMaxTbHits_Rstrip[i_rstrip]->GetYaxis()->SetRangeUser(0.5,1.5);
      h_mMeanMaxTbHits_Rstrip[i_rstrip]->GetYaxis()->SetLabelSize(0.08);
      h_mMeanMaxTbHits_Rstrip[i_rstrip]->GetYaxis()->SetNdivisions(505);
      h_mMeanMaxTbHits_Rstrip[i_rstrip]->SetLineColor(i_rstrip+1);
  ...
  string output_stop = "./figures/SignalTS_140V.pdf]";
