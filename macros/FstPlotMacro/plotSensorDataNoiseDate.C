...
void plotSensorDataNoiseDate(string mod = "Mod03", string hv = "HV140V")
{
  const int defTimeBin = 0;
  std::vector<string> date;

  cout << "Read in run list->" << endl;
  string inputfile = Form("../../list/FST/Date_%s_%s.list",mod.c_str(),hv.c_str());
  ...
  {
    string input_Sensor = Form("../../output/noise/FstDataNoise_%s_%s_%s.root",mod.c_str(),hv.c_str(),date[i_date].c_str());
  ...
  string FigName = Form("./figures/DataNoiseDate_%s_%s.pdf",mod.c_str(),hv.c_str());
